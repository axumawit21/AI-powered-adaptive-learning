<template>
  <aside
    class="w-64 h-screen flex flex-col glass-panel border-r relative overflow-hidden"
  ...
  >
    ...
    <div class="flex-1 flex flex-col min-h-0 z-10 px-4 py-6">
      <!-- Header -->
        ...
        <div class="flex items-center justify-between px-2 mb-3">
          ...
          >
            ...
            <button
              ...
              :class="{ 'pointer-events-none': isSearchOpen && !searchQuery }"
              title="Search"
            >
              <svg
    ...
    >
      ...
      >
        ...
        <span class="hidden xl:inline" style="color: var(--text-secondary)"
          >Settings</span
        >
      </button>
...
const groupedHistory = computed(() => {
  ...
  props.history.forEach((session) => {
    ...
    // Normalize subject names if needed
    if (!groups[subject]) {
      groups[subject] = {
        subject,
        chats: [],
...
function formatSubjectName(name) {
  if (!name) return "";
  // 1. "history9" -> "history 9" (insert space between letter and digit)
  // 2. "math_101" -> "math 101" (replace underscore)
